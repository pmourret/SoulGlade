...
- **Pastille « ADULTE ARMÉ »** : lire l'interrupteur NSFW du personnage courant depuis la fiche (`/api/character`). S'il n'y est pas exposé, l'ajouter au schéma de réponse côté `routers/state` (lecture seule, test d'isolation entre deux personnages), puis régénérer les types OpenAPI.
