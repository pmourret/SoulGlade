...
## Amendements décidés à l'implémentation (23/09/2026)

Trois points où le code s'écarte de ce qui est écrit plus bas. Ils sont ici
plutôt que corrigés dans le texte, pour que la spec validée reste lisible.

1. **Le contrat `data-s` change de grain.** §S1 demandait que « chaque module
   garde `data-s={key}` ». Avec catégories + sous-barre, les huit destinations
   ne sont plus jamais dans le DOM en même temps, donc le contrat se coupe en
   deux : `data-s` porte la **catégorie** (trois valeurs), `data-m` porte le
   **module** (sept). `.tabs` garde son nom et nomme la barre de catégories.
2. **`#btnFocus` va dans la sous-barre**, pas dans le menu d'identité (§S6
   laissait les deux ouverts). La sous-barre survit au mode focus : entrer et
   sortir se font au même endroit, ce qu'un bouton dans l'en-tête ne permet
   pas puisque l'en-tête disparaît.
3. **Le bandeau de modifications n'affiche pas de N.** §S5 demande « N
   modifications non enregistrées » ; `useScenes` expose `dirty` en booléen,
   pas un compte, et il n'y a pas de N honnête à écrire. Même règle que celle
   que la spec applique elle-même à « dernière réponse il y a N s ». En
   revanche le raccourci `Ctrl S`, que §S5 demande d'**afficher**, n'existait
   nulle part dans le frontend : il est **câblé** plutôt que seulement dessiné.
