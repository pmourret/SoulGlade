...
## Vérifications du mode Plan (25/09/2026)

Lues dans le code avant d'écrire la moindre ligne. Elles ajustent la spec
là où elle posait une question.

- **Portraits du registre.** La route de base gelée existe : `GET /img/base`
  (et non `/api/character/base`). Elle ne prend aucun nom de fichier : elle
  lit `base_gelee` dans le `config.json` du personnage nommé par
  `?character=`, validé par `RequiredCharacterId`. `test_base_isolation.py`
  vérifie qu'un personnage A ne reçoit jamais la base de B, même en forçant
  `name=` ou `base_gelee=` dans la requête. L'appeler avec
  `?character={row.id}` est donc sûr. Un personnage sans base, ou au pack
  inconnu (la dépendance le refuse en 400), retombe sur l'initiale.
- **Ce que `/api/characters` renvoie** : `id`, `name`, `type`, `world`
  (`WorldBrief`), `nsfw`, `content_types`, `known_universe`. **Ni style, ni
  pack, ni teinte d'accent.** Donc pas de colonne Style, pas de ligne Pack ni
  Style dans l'aperçu, et l'initiale prend `--acc`.
- **Clic sur un personnage au pack inconnu, aujourd'hui** : il fait entrer
  comme les autres (`selectCharacter` vers Produire). Chaque route bornée au
  personnage rend alors 400 (`ss.character` refuse le pack), et les écrans
  affichent l'erreur. Comportement inchangé : l'aperçu l'annonce, la ligne
  reste un lien.
- **Portraits non retenus** : les candidats générés restent dans
  `ComfyUI/output/`, et une base gelée ou fournie est copiée dans
  `ComfyUI/input/<CID>_BASE.<ext>`. Rien ne les efface à l'abandon. La
  confirmation de sortie dit donc : « Les portraits déjà générés restent dans
  le dossier de sortie de ComfyUI. » Elle ne s'affiche que sur le retour de la
  barre d'écran : le routeur n'a pas de garde de navigation, et en ajouter une
  pour ce seul écran sort du périmètre.
- **Fumigations** : `test_characters.js` lit `#charGrid`, `[data-char-card]`
  (`code` pour l'identifiant, `href*=`), `data-new`, `data-current`, et exige
  au moins deux `data-char-tag` par carte (type et monde). En liste, type et
  monde deviennent des colonnes : la vérification [2] lit ces cellules à la
  place. `test_wizard.js` passe à cinq étapes : Identité en premier, sa
  condition (nom et identifiant valide) testée à cette étape, et le message
  « Renseigne d'abord un identifiant » de l'étape Base disparaît avec sa
  vérification [8]. Les cartes d'option perdent `.it` : le test les lit par
  `[role="radio"]`.

## Audit en vrai (25/09/2026)

Captures à 1440 et 1024 sur tous les cas du critère de sortie, génération
réelle de 4 portraits comprise (220 s). Ce que la lecture du code ne
montrait pas :

- **Majeur, antérieur à l'écran.** Choisir un personnage n'entrait pas dans
  Produire : `selectCharacter` posait le state avant que le routeur ait
  validé la nouvelle adresse, et le miroir d'URL réécrivait la requête sur
  l'ancien chemin, ce qui ramenait au sas. La création tombait dans la même
  course. Corrigé dans `CharacterContext` : le miroir attend l'adresse
  demandée. La fumigation du registre vérifie maintenant le chemin.
- **Majeur, antérieur.** Le gel d'un portrait prend quelques secondes.
  Pendant ce temps, un identifiant changé ne demandait aucune confirmation,
  puis la réponse tardive rattachait une base écrite sous l'ancien
  identifiant. Une réponse qui ne correspond plus à l'identifiant courant
  est désormais ignorée, et le gel en cours s'affiche.
- **Moyen.** Les colonnes Type et Monde ne s'alignaient pas d'une ligne à
  l'autre (pistes dépendantes du contenu). Les pistes sont maintenant fixes.
- **Mineur.** Le nom de l'aperçu et les titres d'étape héritaient des
  capitales du `h2` global. Pack inconnu : le portrait n'est plus demandé
  (la route répond 400), l'initiale s'affiche directement.

Reste ouvert, hors périmètre : « Relancer 4 portraits » reste actif pendant
une génération (déjà le cas avant), et `POST /api/characters` accepte une
base dont le nom ne dérive pas de l'identifiant.
