...
Trois, tous relevés en lisant `AUTOMATION/entrainement.py` et
`api/services/training.py` avant de dessiner, comme §S3 et §S6.4 le demandent.

1. **La barre de répartition ne porte que trois parts, et les jamais étiquetées
   passent en sous-bande.** `candidats` sépare la file des écartées par un
   `continue`, et range `sans_etiquette` et `sans_fichier` comme deux
   sous-ensembles de la file dont la réponse ne dit jamais comment ils se
   croisent. Une surcouche hachurée posée sur la gauche de la barre affirmerait
   que les jamais étiquetées sont exportables, ce que rien ne garantit. La barre
   porte donc exportables, sans fichier et écartées, trois parts disjointes dont
   la somme n'est écrite nulle part ; une sous-bande de 4 px alignée sur la seule
   largeur de la file porte les jamais étiquetées. Tranché avec Pierre pendant
   le plan.

2. **L'ancre réinjectée et la famille de modèle sont absentes du panneau
   d'export.** `_pour_le_fil` ne les expose pas : elles n'existent que dans la
   réponse d'un export et dans les manifestes passés. §S6.2 prévoit ce cas et
   demande l'absence de la mention plutôt qu'une valeur devinée. Le déclencheur,
   lui, est exposé et prend la ligne. Les deux restent visibles dans
   l'historique, qui les a.

3. **L'aperçu de dossier nomme les fichiers réellement écrits.** Ce n'est pas
   `TRAINING/{personnage}/{date}/` avec un `img/`, mais
   `PROD/_ENTRAINEMENT/{personnage}/{AAAAMMJJ-HHMMSS}/` avec
   `dataset/{répétitions}_{déclencheur}/`. `manifeste.json` est toujours écrit ;
   `dataset.toml` et `entrainer.sh` seulement si la famille est connue ;
   `kohya_config.json` seulement si cette famille a un preset amont, ce que sdxl
   n'a pas par décision écrite dans `RECETTES`.

Un quatrième point, de vocabulaire : `--danger` n'existe pas dans `tokens.css`.
La couleur pleine s'appelle `--bad`, et `--danger-bg` / `--danger-line` /
`--danger-txt` sont la famille de bandeau. Les parts de barre et le verdict
« manque » prennent `--bad`.
